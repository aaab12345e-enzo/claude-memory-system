...
> 🔖 **Consolidation marker.** **Consolidated up to here @ <date> <trigger>.** This pass covers: <the completed tasks folded into the marker>. ⚠️ **Iron rule**: the marker always leaves **the latest task outside the marker** as the "live tail" (keep one even if it's already wrapped up, otherwise the live tail is empty → infinite-loop bug). Resume rule: original dialogue **before** this marker = trash (trust only the memory library), **after** it = live tail, read it normally.
...
> **System architecture.** DB (the full set on disk = authoritative) / working memory (context = discardable cache); topics = locality → the working set stays small; three triggers (boundary / scheduled / capacity) maintain the working set; iron rule flush-before-evict. Long conversations = the three-marker chain (drop the marker → treat everything before the marker as trash, don't reason over it → when it won't fit, drop that first). Full text [[memory-system-architecture]].
