...
**Submodule split threshold (set by the user 2026-07-04)**: the original design had a "project-module-**submodule**" level; the implementation flattens it for now into project-module → event (the submodule tier is carried by sections inside the map). **Stay flattened, split by content volume**: once one module map has **≥5** event leaves under it, or the map body runs **over 100 lines** → that module is due to split out a submodule tier; when the line is hit, I propose it and the user decides. Tree depth grows with the content — don't pre-scaffold it.
